...
func main() {
	...
	files, err := generateRepoConfig(ctx, *configDest, *configSource)
	if err != nil {
		log.Fatal(err)
	}
	for _, f := range files {
...
func generateRepoConfig(ctx context.Context, configDest, configSource string) ([]string, error) {
	ctx, task := trace.NewTask(ctx, "generateRepoConfig")
	defer task.End()
	...
	repos, repoFileMap, err := repo.ListRepositories(ctx, sourceFile)
	if err != nil {
		return nil, err
	}
